...
/**
 * 获取协作服务器 URL
 * - HTTPS 或端口 80/443：走同源，由 nginx 代理 /socket.io 到 3002（因 3002 无 SSL）
 * - HTTP 且其他端口：直连 hostname:3002
 */
const getCollabServerUrl = () => {
  const configured = import.meta.env.VITE_APP_WS_SERVER_URL || "";
  if (configured && !configured.includes("localhost")) return configured;
  if (typeof window === "undefined") return configured;

  const { protocol, hostname, port } = window.location;
  // HTTPS 或标准端口：协作走 nginx 代理，避免直连无 SSL 的 3002
...
class Collab extends PureComponent<CollabProps, CollabState> {
  ...
  saveCollabRoomToFirebase = async (
    ...
    // 自托管模式：禁用 Firebase 持久化，避免反复报错
    const isSelfHosted =
      import.meta.env.VITE_APP_DISABLE_FIREBASE_PERSISTENCE === "true" ||
      (typeof window !== "undefined" &&
        !window.location.hostname.includes("excalidraw.com"));
    if (isSelfHosted) {
      return;
    }
  ...
  startCollaboration = async (
    ...
    if (existingRoomLinkData) {
      ...
      try {
        ...
        this.setErrorDialog(err?.message || String(err));
        return null;
      }
